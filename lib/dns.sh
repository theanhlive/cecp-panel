...
dns_cf_api() {
  ...
  args+=(-H "Authorization: Bearer ${CF_API_TOKEN}" -H "Content-Type: application/json")
  [[ -n "$body" ]] && args+=(-d "$body")
  curl "${args[@]}"
}
...
")"
  local body
  body="$(python3 -c "import json; print(json.dumps({'type':'A','name':'$fqdn','content':'$ip','proxied':$proxied_flag,'ttl':1}))")"
  if [[ -n "$existing" ]]; then
