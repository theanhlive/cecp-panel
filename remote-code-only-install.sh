...
if command -v rsync >/dev/null 2>&1; then
  rsync -a --delete --exclude 'dist/' --exclude '.git/' --exclude 'etc/' --exclude 'bin/' "$src/" /opt/cecp-panel/
else
...
for f in /var/lib/cecp-panel/sites/*.json; do
  d="$(python3 -c 'import json,sys; print(json.load(open(sys.argv[1]))["domain"])' "$f" 2>/dev/null || true)"
  [[ -n "$d" ]] || continue
